...
`lmde7-fde`: a three-phase Ansible project that builds a LUKS2 full-disk-encrypted
LMDE 7 ("Gigi", Debian 13/trixie base) system. Read `README.md` for the user-facing
story, `VERIFY.md` for the on-device checklist, and `AIDE.md` / `SELINUX.md` /
`KERNEL.md` / `SECUREBOOT.md` for the four subsystems that have their own operating manuals. This file is
the operating manual for *changing* the tree.
...
| `secureboot.yml` | booted, firmware in setup mode | Replaces factory PK/KEK/db with your own. Generating is safe; `secureboot_enroll` commits. Needs firmware keys cleared first |
...
cd <repo> && /run/user/$(id -u)/lintenv/bin/python3 -m ansiblelint --offline
```
...
Two commands in this tree change behaviour based on files they find rather than
flags they are given. Both cost a debugging session:
...
- This workstation *is* LMDE 7 (`/etc/os-release` → `gigi`), so package facts can be
  checked locally with `apt-cache policy` / `apt-get download` + `dpkg-deb -x`
  instead of guessing at Debian packaging behaviour. Do that before writing rules
  that depend on a package's layout.
- `/tmp` is mounted `noexec` (this project's own fstab policy), so a binary
  extracted into the scratchpad cannot be executed. Read the shipped man pages
  (`man -l <extracted>/usr/share/man/...`) instead.
- The repo is not a git repository — there is no history to consult and no commits
  to make.
- Stray directory `{inventory,group_vars,roles` at the tree root is fallout from a
  brace-expansion `mkdir` that ran without brace expansion. It is inert; leave it
